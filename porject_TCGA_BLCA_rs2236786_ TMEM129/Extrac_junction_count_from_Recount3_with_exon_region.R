# Goal: extract junction count from Recount3 dataset. Target: TMEM129 and FGFR3 
...
gtf <- import('gencode.v39.annotation.gtf.gz')      # Use your downloaded GTF file
...
exon_df.u <- exon_df %>% filter(transcript_name == "FGFR3-205")

# exon_df.u <- exon_df[!duplicated(exon_df[, c("exon_id", "location")]), ]

FGFR_iso <- exon_df$transcript_name %>% unique() %>% sort()
...
df_final <- df_final[order(df_final$location),]
df_final.t <- as.data.frame(t(df_final))
names(df_final.t) <- df_final.t[1,]
df_final.t <- df_final.t[-1,]
df_final.t[] <- lapply(df_final.t[], as.numeric)
df_final.t <- df_final.t %>% mutate(TCGA_ID=rownames(df_final.t),.before = 1)
