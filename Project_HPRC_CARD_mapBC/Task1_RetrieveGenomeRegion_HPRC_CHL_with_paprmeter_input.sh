...
PATH_TO_FILES="/fdb/HPRC/r2/assemblies"
#OUTDIR="/data/Prokunina_Group/GenomeAssemblies/HPRC/retrievedRegions"

OUTDIR="/data/leec20/GWAS_COVNET_related/HPRC_COVNET_get_bams/Retrive_regions"
...
if [ "$runmod" = "local" ]; then
  ...
  # 3 sample named *_hprc_r2_v1.1.0.fa.gz (HG01978,HG02257,HG03516), so some file are missed. and will need to check 
  # HG002 is named as hg002v1.1xxx.gz well...
  for i in $(ls "$PATH_TO_FILES"/*/*_hprc_r2_v1.0.1.fa.gz); do
    filename=$(basename "$i" _hprc_r2_v1.0.1.fa.gz)
  ...
  swarm -f cmdfile_$genomicRegion -t $nthread -g 80 --logdir logs_$genomicRegion  --job-name $genomicRegion --time=03:00:00 --module seqkit --partition quick 
fi
